...
router.get('/data-list', async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT id as dataId, 
       CASE 
         ...
         WHEN gas IS NOT NULL THEN 'pressure' 
         ELSE 'all' 
       END as type, 
       COALESCE(temperature, humidity, gas) as value, 
    ...
    const [countRows] = await db.query(
      `SELECT COUNT(*) as total FROM environment_data ${whereClause}`,
      params
    );
...
router.post('/export', async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT record_time as timestamp, temperature, humidity, gas as pressure, co2, pm25 
       FROM environment_data 
       ${whereClause} 
    ...
    if (format === 'xlsx') { 
        ...
        try {
          ...
          if (!worksheet) {
            ...
            worksheet.columns = [
              ...
              { header: '气压(hPa)', key: 'pressure', width: 15 },
              { header: 'CO2(ppm)', key: 'co2', width: 15 },
              { header: 'PM2.5(μg/m³)', key: 'pm25', width: 15 }
            ];
          }
        } catch (error) {
      ...
      let csvContent = '';
      if (!fs.existsSync(filePath)) {
        csvContent = '时间,温度(°C),湿度(%),气压(hPa),CO2(ppm),PM2.5(μg/m³)\n';
      }
      rows.forEach(row => {
        csvContent += `${row.timestamp.toISOString()},${row.temperature},${row.humidity},${row.pressure},${row.co2},${row.pm25}\n`;
      });
  ...
  try {
    ...
    if (type === 'all' || type === 'temperature') {
      const [tempStats] = await db.query(
        'SELECT AVG(temperature) as avg, MAX(temperature) as max, MIN(temperature) as min, COUNT(*) as count FROM environment_data WHERE record_time BETWEEN ? AND ? AND temperature IS NOT NULL',
        [startTime, endTime]
      );
    ...
    if (type === 'all' || type === 'humidity') {
      const [humidityStats] = await db.query(
        'SELECT AVG(humidity) as avg, MAX(humidity) as max, MIN(humidity) as min, COUNT(*) as count FROM environment_data WHERE record_time BETWEEN ? AND ? AND humidity IS NOT NULL',
        [startTime, endTime]
      );
    ...
    if (type === 'all' || type === 'pressure') {
      const [pressureStats] = await db.query(
        'SELECT AVG(gas) as avg, MAX(gas) as max, MIN(gas) as min, COUNT(*) as count FROM environment_data WHERE record_time BETWEEN ? AND ? AND gas IS NOT NULL',
        [startTime, endTime]
      );
...
router.get('/trends', cacheMiddleware(600), async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT DATE_FORMAT(record_time, '${timeFormat}') as label, AVG(${type}) as value 
       FROM environment_data 
       WHERE record_time BETWEEN ? AND ? AND ${type} IS NOT NULL 
       GROUP BY label 
       ORDER BY label`,
      [startTime, endTime]
    );
